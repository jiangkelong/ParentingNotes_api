...
namespace ParentingNotes_Api.Common
{
    public class Utils
    {
        ...
        /// <returns>时：分</returns>
        public static string TwoTimeInterval(string beginTime,string endTime)
        {
            //开始时间
            var date1 = DateTime.Parse(beginTime);
            //结束时间
            var date2 = DateTime.Parse(endTime);

            //相差毫秒数
            var dateDiff = (int)(date2 - date1).TotalMilliseconds;

            //计算小时数
            var hours = dateDiff / (3600 * 1000);

            // 计算相差分钟数
            var residue = dateDiff % (3600 * 1000); // 计算小时数后剩余的毫秒数
            var minutes = residue / (60 * 1000);

            var result = "";
            result += hours > 0 ? hours + "小时" : "";
            result += minutes > 0 ? minutes + "分钟" : "";

            result = string.IsNullOrEmpty(result) ? "不足1分钟" : result;
